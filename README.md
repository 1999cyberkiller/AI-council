...
它会把同一个问题交给 4 个独立模型。每个模型只看到自己的工具包，输出判断、关键假设、会改变观点的触发条件和风险。系统最后只做加权综合，并保留共识、分歧和少数派意见。
...
在 `src/council-config.js` 里增加模型。

每个模型都可以配置 `allowedTools`。模型只能看到自己白名单内的工具结果。
